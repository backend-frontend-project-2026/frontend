...
import type { User, UserBudget, UserHabits, SleepSchedule, CleanlinessLevel, NoiseLevel, GuestFrequency, PetPreference, SmokingPreference, AlcoholPreference, RoomOrderPreference, StayDuration, HousingType, Gender } from '@/entities/user';
...
function mapHabits(profile: ProfileResponse): UserHabits {
  return {
    ...
    noiseLevel: (profile.noise_level as unknown as NoiseLevel) ?? 'moderate',
    ...
    alcoholPreference: (profile.alcohol_preference as unknown as AlcoholPreference) ?? 'rarely',
    roomOrderPreference: (profile.room_order_preference as unknown as RoomOrderPreference) ?? 'balanced',
...
export function mapProfileToUser(profile: ProfileResponse): User {
  ...
  return {
    ...
    name: profile.name,
    age: profile.age,
    gender: (profile.sex as Gender) === 'male' ? 'male' : 'female',
    housingType: (profile.housing_type as unknown as HousingType) ?? 'rental',
