...
export function getRoomOrderLabel(user: FullProfile): string {
...
export function getCleanlinessLabel(user: FullProfile): string {
  if (user.habits.cleanliness === 'high') return 'Аккуратно';
  if (user.habits.cleanliness === 'medium') return 'Средне';
  return 'Не важно';
}

export function getGuestsLabel(user: FullProfile): string {
...
export function getQuietHoursLine(user: FullProfile) {
...
export function getStayShort(value: FullProfile['stayDuration']) {
  switch (value) {
    case '1-3 months':
      return '1–3 мес';
    case '3-6 months':
